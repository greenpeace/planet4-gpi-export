...
function replace_attachment($text, $attachment) {
	...
	$basename = str_replace("(", "", urldecode($basename));
	$basename = str_replace(")", "", urldecode($basename));
	$bodytag = str_replace($attachment, "https://storage.googleapis.com/planet4-thailand-stateless-release/2019/03/". $basename , $text);
	return $bodytag;
}
add_action('pmxi_saved_post','post_saved',10,1);
function post_saved( $postid ) {
	$local_path = 'https://release.k8s.p4.greenpeace.org/thailand/wp-content/uploads/';
	$gcs_path   = 'https://storage.googleapis.com/planet4-thailand-stateless-release/';
